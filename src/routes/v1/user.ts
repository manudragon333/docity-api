...
import {
  handleUserActionPermission,
  isSuperAdmin,
  isSuperAdminOrCivilEngineerorUser,
} from "middlewares/permission";

const router = Router();

router
  .route(USER_PATHS.CIVIL_ENGG_LIST)
  .get(UserValidation.getCivilEngg, UserController.getCivilEnggList);
...
router
  .route(USER_PATHS.USER_INSTANCE)
...
router
  .route(USER_PATHS.PROFILE_IMAGE)
  .put(
    isSuperAdmin,
...
router
  .route(USER_PATHS.ATTACHMENT)
  .get(isSuperAdmin, UserValidation.doNothing, UserController.getUserIdProofs)
  .post(
    isSuperAdmin,
    uploadUserDocuments,
    UserValidation.addAttachment,
    UserController.addUserIdProof
  );
router
  .route(USER_PATHS.UPDATE_STATUS)
  .put(
    handleUserActionPermission,
    UserValidation.handleUserAction,
    UserController.handleUserAction
  );
router
  .route(USER_PATHS.INVITE_USER)
  .post(isSuperAdmin, UserValidation.inviteUser, UserController.inviteUser);
router
  .route(USER_PATHS.SHARE_REPORT)
  .post(
    isSuperAdminOrCivilEngineerorUser,
    UserValidation.shareReport,
    UserController.shareReport
  );
