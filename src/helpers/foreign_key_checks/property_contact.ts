import {APIError, IPropertyContact, IServiceResponse, Log} from 'models';
...
export async function verifyPropertyContactForeignKeys(serviceResponse: IServiceResponse, payload: IPropertyContact) {
    try {
        if (payload?.id) {
            // TODO
        }
        ...
        Log.error(TAG, 'verifyPropertyContactForeignKeys()', error);
    }
}
