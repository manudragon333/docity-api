...
export function searchTextValidation() {
  return Joi.string()
    .min(MIN_SEARCH_TEXT_LENGTH)
    .messages({
      "any.min":
        "Search text field should be minimum of " + MIN_SEARCH_TEXT_LENGTH,
    });
}
...
export function idValidation(field?: string) {
  return Joi.object().keys({
    id: uniqueIdentifiedValidation()
      .required()
      .messages({
        "any.required": ErrorMessages.IS_REQUIRED.replace(
          "$field",
          field || "id"
        ),
        "string.pattern.base": ErrorMessages.INVALID_NUMBER_STRING.replace(
          "$field",
          field || "id"
        ),
      }),
  ...
  return Joi.object().keys({
    phoneCountryCode: Joi.string()
      ...
      .messages({
        ...
        "any.max": ErrorMessages.INVALID_LENGTH.replace(
          "$field",
          "phoneCountryCode"
        ).replace("$length", "5"),
      }),
    phoneNumber: Joi.string()
      ...
      .messages({
        ...
        "string.pattern.base": ErrorMessages.INVALID_VALUE.replace(
          "$field",
          "phoneNumber"
        ).replace("$value", "mobile number with 10 digits."),
      }),
  });
}
...
export function addressSchema() {
  return Joi.object().keys({
    ...
    district: idValidation("district.id").messages({
      "any.required": ErrorMessages.IS_REQUIRED.replace("$field", "district"),
    }),
  });
}
...
const buildUsefulErrorObject = (errors: any): IServiceResponse => {
  ...
  for (const error of errors.error.details) {
    if (!usefulErrors.hasOwnProperty(error.path.join("_"))) {
      usefulErrors.push(
        new APIError(error.message, error.type, error.path.join("_"))
      );
    } else {
  ...
  return new ServiceResponse(
    ...
    "Please fill all Mandatory fields with valid values!.",
    null,
    true,
    usefulErrors
  );
...
export async function validate(schema, req, res, next) {
  try {
    ...
    if (result.error) {
      Log.debug("result.error: ", JSON.stringify(result));
      const errorResponse: IServiceResponse = buildUsefulErrorObject(result);
      res
        .status(errorResponse.statusCode || HttpStatusCodes.BAD_REQUEST)
        .send({ errors: errorResponse.errors });
    } else {
      next();
    }
  } catch (error) {
    Log.error(TAG, "validate()", error);
    next(error);
  }
}

export async function validate2(schema, req, res, next) {
  try {
    Log.info("START of common.validator.validate()");
    // schema = schema.append({
    //   token: Joi.string().allow(""),
