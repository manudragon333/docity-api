...
export async function saveContactMeRequest(
  loggedInUser: IUserSession,
  payload: IPropertyContact
): Promise<IServiceResponse> {
  Log.info(TAG + ".saveContactMeRequest()");
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.CREATED,
    "Successfully created contact me. Our Agent will contact you soon.",
    true
  );
  try {
    ...
    serviceResponse.data = await PropertyContactData.saveContactMeRequest(
      loggedInUser,
      payload
    );
  } catch (error) {
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("save", "contactMeRequest")
    );
  }
  return serviceResponse;
}
...
export async function saveVerifyProperty(
  loggedInUser: IUserSession,
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.CREATED,
    "Successfully created verify property request.",
    true
  );
  let session;
  try {
    ...
    const sequenceNubSeries =
      await SequenceData.fetchAndUpdateLatestSequenceNumber(
        SEQUENCE_TYPES.PROPERTY_REQUEST.name,
        SEQUENCE_TYPES.PROPERTY_REQUEST.default_series
      );
    payload.referenceId =
      sequenceNubSeries.series +
      addPaddingZeros(sequenceNubSeries.nb, MAX_SEQUENCE.toString().length);
    ...
    serviceResponse.data = await PropertyRequestData.saveVerifyProperty(
      payload,
      loggedInUser
    );
  } catch (error) {
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("verify", "property")
    );
  }
  if (serviceResponse?.errors?.length) {
    Log.debug("ROLLBACK");
...
export async function updateVerifyPropertyRequest(
  loggedInUser: IUserSession,
  payload: IPropertyRequest
): Promise<IServiceResponse> {
  Log.info(TAG + ".saveVerifyProperty()");
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.CREATED,
    "Successfully updated verify property request.",
    true
  );
  let session;
  try {
    ...
    serviceResponse.data =
      await PropertyRequestData.updateVerifyPropertyRequest(
        payload,
        loggedInUser
      );
  } catch (err) {
...
export async function getAllPropertyRequests(
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.CREATED,
    "Successfully fetched property requests.",
    true
  );
  try {
    const records = await PropertyRequestData.getAllPropertyRequests(
      loggedInUser,
      queryParams
    );
    const recordCount = await PropertyRequestData.getAllPropertyRequestsCount(
      loggedInUser,
      queryParams
    );
    serviceResponse.data = new ListAPIResponse(
      records,
      recordCount > queryParams.limit,
      queryParams.offset,
      queryParams.limit,
      recordCount,
      queryParams.sortBy,
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("fetch", "properties")
    );
  }
  return serviceResponse;
}

export async function getAllPropertyRequestLocations(
  loggedInUser: IUserSession,
  search: string
) {
  Log.info(TAG + ".getAllPropertyRequests()");
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.CREATED,
    "Successfully fetched property request locations.",
    false
  );
  try {
    let result = await PropertyRequestData.getAllPropertyRequestLocations(
      loggedInUser,
      search
    );
...
export async function getAllPropertyContacts(
  loggedInUser: IUserSession,
  queryParams: IBaseListAPIRequest
) {
  Log.info(TAG + "getAllPropertyContacts()");
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully fetched the property contact requests.",
    false
  );
  try {
    const result = await PropertyContactData.getAllPropertyContactMeRequests(
      loggedInUser,
      queryParams
    );
    serviceResponse.data = new ListAPIResponse(result);
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("fetch", "contacts")
    );
  }
  return serviceResponse;
}
...
export async function savePropertyDocs(req: any, res: any) {
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully saved the property document.",
    false
  );
  try {
    const isPropertyExist = await PropertyRequestData.checkPropertyRequestExist(
      propertyRequestID
    );
    if (isPropertyExist) {
      ...
      if (body.attachmentType) {
        ...
        if (attachmentType) {
          ...
          for (const file of req.files) {
            const saveFile = propertyDocDataMapping(
              propertyRequestID,
              attachmentType,
              file,
              loggedInUser
            ...
            promises.push(
              PropertyRequestData.savePropertyDocument(loggedInUser, saveFile)
            );
          }
      ...
      serviceResponse.addError(
        new APIError(
          "Property request does not exist!!",
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("save", "property docs")
    );
  }
...
export async function linkPropertyDoc(req: any, res: any) {
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully linked the property document.",
    false
  );
  try {
    ...
    if (body.attachmentType) {
      ...
      if (attachmentType) {
        ...
        const doc = await PropertyRequestData.savePropertyDocument(
          loggedInUser,
          saveFile
        );
        doc.path = await getFileURL(doc.path, AWS_S3.docBucket);
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("save", "property doc")
    );
  }
  if (serviceResponse?.errors?.length && req.files) {
    deleteSavedFiles(req.files).catch((error) => {
...
export async function deletePropertyDoc(id: string) {
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully deleted the property document.",
    true
  );
  let session;
  try {
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("delete", "property docs")
    );
  }
  if (serviceResponse?.errors?.length) {
    Log.debug("ROLLBACK");
...
export async function getPropertyDocs(propertyRequestId: string) {
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully fetched the property documents.",
    false
  );
  try {
    const result = await PropertyRequestData.getPropertyDocuments(
      propertyRequestId
    );
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("fetch", "property docs")
    );
  }
  return serviceResponse;
}

export async function updatePropertyDocNotes(
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully updated the property document notes.",
    true
  );
  try {
    await PropertyRequestData.updatePropertyDocument(propertyDocId, notes);
    serviceResponse.data = await PropertyRequestData.getPropertyDocument(
      propertyDocId
    );
  } catch (error) {
    Log.error(TAG, "updatePropertyDocNotes()", error);
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("update", "property docs")
    );
  }
  return serviceResponse;
}

export async function updatePropertyDocumentWriter(
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully saved the property document writer details.",
    true
  );
  try {
    await PropertyRequestData.updatePropertyDocumentWriter(
      propertyRequestId,
      documentWriter
    );
    serviceResponse.data = await PropertyRequestData.getPropertyRequestById(
      propertyRequestId
    );
  } catch (error) {
    Log.error(TAG, "updatePropertyDocumentWriter()", error);
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("save", "property doc wirter")
    );
  }
  return serviceResponse;
}
...
export async function getPropertyRequestDetails(propertyRequestId: string) {
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully fetched the property document writer details.",
    false
  );
  try {
    const property: any = await PropertyRequestData.getPropertyRequestById(
      propertyRequestId
    );
    if (property) {
      const docs = await PropertyRequestData.getPropertyDocuments(
        propertyRequestId
      );
      for (const doc of docs) {
        doc.path = await getFileURL(doc.path, AWS_S3.docBucket);
      ...
      if (property?.finalReport?.path) {
        property.finalReport.path = await getFileURL(
          property?.finalReport?.path,
          AWS_S3.docBucket
        );
      }
      property.documents = groupBy(docs, "attachmentType.name");
      const payments = await PaymentData.findLatestOrderByPropertyRequest(
        propertyRequestId
      );
      if (payments) {
        payments.status = new BaseRecord(
          STATUS_LIST[payments.status].id,
          STATUS_LIST[payments.status].name
        );
      }
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("fetch", "property request")
    );
  }
  return serviceResponse;
}

export async function assignPropertyRequestToCivilEngineer(
  propertyRequestId: string,
  civilEngineer: IBaseRecord
) {
  ...
  try {
    const propertyRequestDetails =
      await PropertyRequestData.getPropertyRequestById(propertyRequestId);
    ...
    if (user.role.indexOf(ROLE_LIST.CIVIL_ENGINEER.name) !== -1) {
      ...
      // CLUE
      await PropertyRequestData.assignPropertyRequestToCivilEngineer(
        propertyRequestId,
        civilEngineer.id
      );
      serviceResponse.data = await PropertyRequestData.getPropertyRequestById(
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage(
        "assign",
...
export async function updatePropertyRequestCivilEngineerResponse(
  loggedInUser: IUserSession,
  propertyRequestId: string,
  action: string,
  ...
  try {
    ...
    await PropertyRequestData.updatePropertyRequestCivilEngineerResponse(
      propertyRequestId,
      loggedInUser,
      ceStatus,
      estimatedFinishDate,
...
export async function updatePropertyRequestStatus(
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully updated property request status.",
    true
  );
  try {
    ...
    if (status) {
      await PropertyRequestData.updatePropertyRequestStatus(
        propertyRequestId,
        status
      );
      serviceResponse.data = await PropertyRequestData.getPropertyRequestById(
...
export async function getPropertyDocument(id: string) {
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully fetched the property document.",
    false
  );
  try {
...
export async function saveDocumentComment(
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully saved the property document comment.",
    false
  );
  try {
    await PropertyRequestData.savePropertyDocComment(
      loggedInUser,
      propertyDocId,
      comment
    );
    serviceResponse.data = await PropertyRequestData.fetchPropertyDocComments(
      propertyDocId
    );
  } catch (error) {
    Log.error(TAG, "saveDocumentComment()", error);
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("save", "comment")
    );
  }
  return serviceResponse;
}
...
export async function getDocumentComment(
  ...
  try {
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("get", "comments")
    );
  }
  return serviceResponse;
}
...
export async function updateDocumentComment(
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully updated the property document comment.",
    false
  );
  try {
    serviceResponse.data = await PropertyRequestData.updatePropertyDocComment(
      loggedInUser,
      propertyDocId,
      commentId,
      comment
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("update", "comment")
    );
  }
  return serviceResponse;
}
...
export async function deleteDocumentComment(
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully deleted the property document comment.",
    false
  );
  try {
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("delete", "comment")
    );
  }
  return serviceResponse;
}

export async function sendFinalReportMail(
  toEmailId: string,
  userName,
  referenceId: string
) {
  try {
    ...
    const superAdminRole: IBaseRecord = await fetchRoleByName(
      ROLE_LIST.SUPER_ADMIN.name
    );
    const superAdminEmails = await fetchUserEmailsByRole(superAdminRole.id);
    sendEmail(
      new EmailSender(SENDER_EMAIL_ID),
      new EmailRecipient([toEmailId], superAdminEmails),
      "FINAL REPORT",
      mailBody
    ).catch((error) => {
...
export async function updatePropertyRequestFinalReport(req: any, res: any) {
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully saved the final report.",
    false
  );
  try {
    const propertyRequest = await PropertyRequestData.getPropertyRequestById(
      propertyRequestId
    );
    if (propertyRequest) {
      if (propertyRequest?.status?.id == String(STATUS_LIST["6"].id)) {
        ...
        if (req.files.length) {
          ...
          await PropertyRequestData.updateFinalReport(
            req.userSession,
            propertyRequestId,
            file.key ?? file.path,
            note,
          ...
          sendFinalReportMail(
            propertyRequest.emailId,
            propertyRequest.name,
            propertyRequest.referenceId
          ).catch((error) => {
            Log.error(TAG, "sendFinalReportMail()", error);
          });
          serviceResponse.data = {
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("update", "final report")
    );
  }
  return serviceResponse;
}

export async function shareFinalReport(
  loggedInUser: IUserSession,
  propertyRequestID: string,
  payload: any
) {
  ...
  const serviceResponse: IServiceResponse = new ServiceResponse(
    HttpStatusCodes.OK,
    "Successfully shared the final report.",
    false
  );
  const { body, subject, toMails, ccMails, bccMails } = payload;
  try {
    const propertyRequest = await PropertyRequestData.getPropertyRequestById(
      propertyRequestID
    );
    if (propertyRequest) {
      ...
      if (finalReport?.path) {
        const attachments = [];
        if (S3_STORAGE.enabled) {
          const attachment = await readFiles(
            finalReport.path,
            AWS_S3.docBucket
          );
          attachments.push({
    ...
    serviceResponse.addServerError(
      serviceResponse.buildServiceFailedMessage("share", "final report")
    );
  }
  return serviceResponse;
}
