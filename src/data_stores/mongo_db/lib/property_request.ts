...
export async function saveVerifyProperty(
  ...
  try {
    const propertyRequest = new PropertyRequest({
      ...
      civil_engineer: doc.civilEngineer,
    });
...
export async function getAllPropertyRequests(
  ...
  try {
    ...
    if (
      loggedInUser.role.filter((role) => role.name === ROLE_LIST.USER.name)[0]
    ) {
      conditions.requested_user = loggedInUser.userId;
    } else if (
      loggedInUser.role.filter(
        (role) => role.name === ROLE_LIST.CIVIL_ENGINEER.name
      )[0]
    ) {
    ...
    if (queryParams.sortBy) {
      options.sort = {};
      options.sort[queryParams.sortBy] =
...
export async function getAllPropertyRequestsCount(
  loggedInUser: IUserSession,
  queryParams: IPropertyRequestListApiRequest
) {
...
export async function updatePropertyRequestPaymentStatusAndStatus(
  id: string,
...
export async function savePropertyDocComment(
  loggedInUser: IUserSession,
  id: string,
  comment: IDocComment
) {
...
export async function updatePropertyDocComment(
  loggedInUser: IUserSession,
  docId: string,
  commentId: string,
  comment: IDocComment
...
export async function checkValidUserOfPropertyRequest(
  propertyRequestId: string,
  userId: string
) {
  try {
    const result = await findOne(
      ...
      {
        _id: propertyRequestId,
        $or: [{ civil_engineer: userId }, { requested_user: userId }],
      },
      {
        _id: 1,
      }
