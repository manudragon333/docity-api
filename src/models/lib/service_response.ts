...
export interface IServiceResponse {
    ...
    buildServiceFailedMessage?(action: string, entity?: string): string;
}
...
export class ServiceResponse implements IServiceResponse {
    ...
    public buildServiceFailedMessage(action: string, entity: string): string {
        return ErrorMessages.FAILED_SERVICE_RESPONSE.replace('$action', action).replace('$entity', entity);
    }
}
